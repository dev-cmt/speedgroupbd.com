                ...
                <div class="card-body">
                    <form action="{{ route('places.update', $place->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select name="country_id" id="country_id" class="form-control" required>
                                @foreach($countries as $country)
                                    <option value="{{ $country->id }}" {{ $place->country_id == $country->id ? 'selected' : '' }}>
                                        {{ $country->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="description" id="description" class="form-control" rows="4">{{ old('description', $place->description) }}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            @if($place->image)
                                <div class="mb-2">
                                    <img src="{{ asset('public/' . $place->image) }}" width="100" class="rounded">
                                </div>
                            @endif
